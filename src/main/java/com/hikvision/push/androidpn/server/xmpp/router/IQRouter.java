...
import com.hikvision.push.androidpn.server.xmpp.handler.AbstractIQHandler;
import com.hikvision.push.androidpn.server.xmpp.handler.IQAuthHandler;
import com.hikvision.push.androidpn.server.xmpp.handler.IQRosterHandler;
	...
	private void handle(Channel channel, IQ iq, XMLElement childElement,
		...
		try {
			...
			Connection connect = connectionManager
					.getConnectByChannelId(channel.getId());
			if (connect != null) {
				IQ reply = IQUtils.createResultIQ(iq);
