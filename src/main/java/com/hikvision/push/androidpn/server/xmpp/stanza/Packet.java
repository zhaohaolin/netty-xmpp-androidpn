...
import com.hikvision.push.androidpn.server.xmpp.xml.HasXML;
 ...
 * 
 * Basic stanzas have <i>id</i>, <i>from</i> and <i>to</i> attributes, as well
 * as support for XML extensions.
 */
public abstract class Packet implements HasXML {
	 ...
	 */
	protected Packet(final XMLElement xml) {
	 ...
	 */
	protected Packet(final String name) {
	 ...
	 */
	protected Packet(final String name, final String namespace) {
	 ...
	 */
	@Nullable
	public final XMLElement getExtension(final String name,
	 ...
	 */
	public final XMLElement addExtension(final String name,
			final String namespace) {
	...
	public void setError(PacketError error) {
		...
		if (xml.hasChild("error")) {
			xml.removeChild(xml.getFirstChild("error"));
		}
		xml.addChild(error.xml);
	}
	...
	public PacketError getError() {
		XMLElement error = (checkNotNull(xml.getChildren("error"))).get(0);
		if (null != error) {
	 ...
	 */
	@Nullable
	public static final Packet fromElement(final XMLElement element) {
